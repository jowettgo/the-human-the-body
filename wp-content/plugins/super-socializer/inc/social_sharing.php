...
function the_champ_prepare_sharing_html( $postUrl, $sharingType = 'horizontal', $displayCount, $totalShares, $shareCountTransientId ) {
	...
	$originalPostTitle = $postTitle = html_entity_decode( $postTitle, ENT_QUOTES, 'UTF-8' );
		...
		foreach ( $theChampSharingOptions[$sharingType.'_re_providers'] as $provider ) {
			...
			$liItems .= str_replace(
				...
				array(
					...
					$postUrl,
					$postTitle,
					esc_html( str_replace( array('#', '%27', '%22'), array('%23', "'", '"'), urlencode( $originalPostTitle ) ) ),
					$twitterUsername,
				...
				switch($provider){
					case 'facebook':
						if(isset($body -> share) && !empty($body -> share -> share_count)){
							$responseData[$targetUrl]['facebook'] = $body -> share -> share_count;
							$shareCountTransient['facebook'] = $body -> share -> share_count;
						}else{
